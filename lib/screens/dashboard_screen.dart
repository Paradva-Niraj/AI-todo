...
class _DashboardScreenState extends State<DashboardScreen> {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          action: SnackBarAction(
            label: 'Retry',
            onPressed: () {
              // Try fetching again or navigate to login
              _fetchForRange(forceRefresh: true);
            },
          ),
        ),
      );
    }
    // Optionally logout automatically after delay
  ...
  Future<void> _fetchForRange({bool forceRefresh = false, bool showLoading = false}) async {
    ...
    if (res['ok'] == true) {
      ...
      if (res['status'] != 401) _showSnackbar(err.toString(), isError: true);
    }
  }
  ...
  Future<void> _handleComplete(String id, DateTime day) async {
    final dateStr = DateHelper.toIsoDateString(day);
    final res = await TodoService.markComplete(id, date: dateStr);
    if (!mounted) return;
    _checkAuthError(res);
    if (res['ok'] == true) {
      await _fetchForRange(forceRefresh: true);
      if (mounted) _showSnackbar('Marked complete for ${DateFormat('MMM d').format(day)}');
    } else if (res['status'] == 409) {
      await _fetchForRange(forceRefresh: true);
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Already completed for ${DateFormat('MMM d').format(day)}'),
          action: SnackBarAction(
            label: 'Undo',
            onPressed: () => _handleUncomplete(id, day),
          ),
          duration: const Duration(seconds: 4),
        ),
      );
    } else if (res['status'] != 401) {
      _showSnackbar(res['error'] ?? 'Failed to mark complete', isError: true);
    }
  }

  Future<void> _handleUncomplete(String id, DateTime day) async {
    final dateStr = DateHelper.toIsoDateString(day);
    ...
    if (res['ok'] == true) {
      await _fetchForRange(forceRefresh: true);
      if (mounted) _showSnackbar('Completion removed');
    } else if (res['status'] != 401) {
      _showSnackbar(res['error'] ?? 'Failed to undo', isError: true);
    }
  }
  ...
  Future<void> _deleteTodo(String id) async {
    ...
    if (res['ok'] == true) {
      ...
      _showSnackbar(res['error'] ?? 'Failed to delete', isError: true);
    }
  }
  ...
  Future<void> _openEditor({Map<String, dynamic>? todo}) async {
    ...
    if (result == true && mounted) await _fetchForRange(forceRefresh: true);
  }
  ...
  void _showSnackbar(String message, {bool isError = false}) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(content: Text(message), backgroundColor: isError ? Colors.red.shade700 : null, behavior: SnackBarBehavior.floating),
    );
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(onPressed: _openAi, icon: const Icon(Icons.smart_toy), tooltip: 'AI Assistant'),
        ...
        child: GestureDetector(
          onVerticalDragEnd: (details) {
            if (details.primaryVelocity != null) {
              if (details.primaryVelocity! < -400) _openAi(); // swipe up
              if (details.primaryVelocity! > 400) _openAi(); // swipe down (also opens AI)
            }
          },
          child: Column(
            children: [
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              Row(
                                children: [
                                  Text(DateFormat('EEEE').format(currentDay),
                                      style: TextStyle(
                                        fontSize: 14,
                              ...
                              Text(DateFormat('d MMM yyyy').format(currentDay),
